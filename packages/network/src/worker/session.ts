import { type Client, type ClientBroadcast, type SessionManager } from '../api/client'
...
class SessionImpl implements Client {
  ...
  async modify<T, V>(workspaceId: WorkspaceUuid, req: T): Promise<ResponseValue<V>> {
    ...
    return await this.manager.modify(workspaceId, request)
  }
...
export class SessionManagerImpl implements SessionManager {
  ...
  constructor (
    readonly factory: (broadcast: ClientBroadcast) => Node,
  ...
  async register (account: AccountUuid, sessionId: string): Promise<Client> {
    ...
    const session = new SessionImpl(account, sessionId, this.worker, ++this.clientTick)
