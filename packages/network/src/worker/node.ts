...
import type { Node, NodeFactory, NodeManager, Workspace, WorkspaceFactory } from '../api/node'
...
export class NodeImpl implements Node {
  workspaces: Record<WorkspaceUuid, Workspace | Promise<Workspace>> = {}
  constructor (
  ...
  async workspace (workspaceId: WorkspaceUuid): Promise<Workspace> {
  ...
  async askTo<T, V>(req: Request<T>, wrk: Node, workspaces: WorkspaceUuid[], result: RequestAkn): Promise<void> {
  ...
  async modify<T, V>(workspaceId: WorkspaceUuid, req: Request<T>): Promise<ResponseValue<V>> {
    ...
    return await wrk.modify<T, V>(req)
  }
  ...
  async close (): Promise<void> {
    for (const wrk of Object.values(this.workspaces)) {
      if (wrk instanceof Promise) {
        await wrk.then(async (w) => { await w.close() })
      } else {
        await wrk.close()
      }
    }
...
export class NodeManagerImpl implements NodeManager {
  workers: Record<NodeUuid, Node | Promise<Node>> = {}
  constructor (
  ...
  async close (): Promise<void> {
    await Promise.all(Object.values(this.workers).map(async (wrk) => (wrk instanceof Promise ? await wrk : await wrk.close())))
    this.workers = {}
  }
